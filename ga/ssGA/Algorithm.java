...
public class Algorithm
{
  ...
  //private  Random rWithSeed; //Random generator with Seed


  // CONSTRUCTOR
  public Algorithm(Problem p, int popsize, int gn, int gl, double pc, double pm, Random rWithSeed)
  throws Exception
  {
    this.gene_number   = gn;
    this.gene_length   = gl;
    ...
    this.pm            = pm;
    this.problem       = p;                     
    this.pop = new Population(popsize,chrom_length,rWithSeed);// Create initial population
    //this.r             = new Random(seed);
    this.r             = rWithSeed; //random number generator with seed for Montecarlo method
    this.aux_indiv     = new Individual(chrom_length,rWithSeed);

    for(int i=0;i<popsize;i++)
    {
        pop.set_fitness(i,problem.evaluateStep(pop.get_ith(i)));
        pop.compute_stats();
        //Contabilize the initial population evaluations and the associated best fitness
        System.out.print(p.get_fitness_counter()); System.out.print("  ");
        System.out.println(get_bestf());
    }
  }
